...
只要最终评分记录的通过数大于 0，任务顶层 `score_valid` 就为 true，即使生成或评分失败。
原先有效的 0 分仍有效；未评测或无有效统计的 0 分仍无效。
Docker/依赖通道故障、pytest 内部错误或中断、统计不完整、执行超时仍标为评测失败；
`status`、`evaluation_valid` 和嵌套评分诊断保留故障信息，不因有分数而改成成功。
...
评测结束后自动生成 `report.md`（完整报告）、`report.json`（机器可读汇总）和
`report.csv`（逐任务明细）。`main.py` 直接运行和 `eval.sh` 启动均支持；失败和正常中断
也会在收尾时生成。启动器将报告路径和 `full_task_average_score` 写入 `run-state.json`，
并在控制台/`launcher.log` 输出摘要；评测进程的摘要位于 `benchmark.log`。

**主分数为全量任务平均分**：

```text
任务分数 = min(测试通过数 / 官方测试总数, 1)
全量任务平均分 = 所有配置任务的分数之和 / 配置任务总数
```

有正分就计入，即使任务为 `failed`、生成 API 错误或评分超时。无可用分数、未进入评测
及缺失结果的任务计 0，仍占全量分母；原先有效的 0 分仍有效。报告主分数不使用
有效任务子集平均，也不使用用例加权平均。JSON 的分数范围为 0–1，Markdown 显示百分比。
若正分缺少可用分母和历史通过率，报告明确标记无法计算，不静默按 0 分处理。

启动时保存 `task-plan.json`，固定本次全部任务 UUID、模型、任务名和官方测试分母。
报告按 UUID 合并 `result/` 与 `workspaces/*/task_state.json`，不重复统计重试归档。
多模型配置会同时列出各模型的全量平均分。报告保留任务状态、评分有效性和故障阶段。

已有实验可补生成，无需启动 Docker 或请求模型：

```bash
.nl2repo-local/venv/bin/python -m claude_code.report experiments/H100-all-005
```

历史实验没有任务计划时，使用配置恢复全量范围，并从历史日志及结果中恢复分母。
原始结果不修改，报告中的 `original_score_valid` 保留历史值，`score_valid` 使用当前规则。
...
- 生成阶段：上游 API 错误和响应流中断由 Claude Code 在原会话内执行请求级恢复。
  ...
  进程已退出但生成失败时仍对已有产物评分，并保留生成失败标记；有正分时分数有效。
